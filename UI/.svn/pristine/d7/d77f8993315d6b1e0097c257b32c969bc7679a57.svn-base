...
{
    /// <summary>
    /// Producer for sending a Message containing information wether the client
    /// wants to delete a participant
    /// </summary>
    public class DeleteParticipantProducer: BaseProducer
    {
        ...
        public DeleteParticipantProducer(IClientInfo connectionData, NetworkAction action) 
            :base(connectionData, action)
        {
            this.queueTopic = connectionData.SimulationName + MessageTopics._deleteParticipant_queue.ToString("g");
            Init();
        }
