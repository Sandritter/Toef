...
namespace UI.Communication.NMS.Producer
{
    ...
    /// </summary>
    public class TileProducer : BaseProducer
    {
        ...
        /// <param name="converter">converts stuff</param>
        /// <param name="action">action that the consumer is responsible for</param>
        public TileProducer(IClientInfo connectionData, IConverter converter, NetworkAction action)
            :base(connectionData, action)
        {
            this.queueTopic = connectionData.SimulationName + MessageTopics._viewTile_queue.ToString("g");
        ...
        /// <summary>
        /// Sends a Message containing tileinformation to simulation
        /// </summary>
        ...
        public override void Send(NetworkAction action, ISend content)
        {
            if (action == this.action)
            {
                ...
                ViewTile vt = (ViewTile)content;
                string text = converter.ViewTileToString(vt);
